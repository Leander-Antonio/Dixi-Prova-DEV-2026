...
public interface PontoRepository extends JpaRepository<Ponto, Long> {
    ...
    // Histórico apenas com marcações válidas
    List<Ponto> findByDesconsideradaIsFalseAndMomentoBetweenOrderByMomentoAsc(
            LocalDateTime inicio,
            LocalDateTime fim
    );

    // Regra do mesmo minuto
    boolean existsByMomentoBetween(
            LocalDateTime inicio,
            LocalDateTime fim
    );

    // Verifica se existe marcação válida no mesmo minuto
    ...
    // Marcações desconsideradas com motivo (fetch para evitar lazy issues)
    @Query("""
